...
public class EditUser {
  ...
  public void testEditUser() throws Exception {
    ...
    for (int second = 0;; second++) {
    	if (second >= 60) fail("timeout");
    	try { if (isElementPresent(By.xpath("//a[contains(text(),'test3/******')]"))) break; } catch (Exception e) {}
    	Thread.sleep(1000);
    }

    driver.findElement(By.xpath("//a[contains(text(),'test3/******')]")).click();
    for (int second = 0;; second++) {
    	if (second >= 60) fail("timeout");
    ...
    for (int second = 0;; second++) {
    	if (second >= 60) fail("timeout");
    	try { if (isElementPresent(By.name("_.password"))) break; } catch (Exception e) {}
    	Thread.sleep(1000);
    }
