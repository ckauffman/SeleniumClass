...
public class AddUser {
  ...
  public void testAddUser() throws Exception {
    ...
    for (int second = 0;; second++) {
    	if (second >= 60) fail("timeout");
    	try { if (isElementPresent(By.xpath("//a[contains(text(),'Add Credentials')]"))) break; } catch (Exception e) {}
    	Thread.sleep(1000);
    }
    ...
    for (int second = 0;; second++) {
    	if (second >= 60) fail("timeout");
    	try { if (isElementPresent(By.cssSelector("select.setting-input.dropdownList"))) break; } catch (Exception e) {}
    	Thread.sleep(1000);
    }
    ...
    driver.findElement(By.name("_.username")).sendKeys("test5");
